...
missing_labels = y_test.isnull()
X_test = X_test[~missing_labels]
y_test = y_test[~missing_labels]

# Clean labels in y_train and y_test
y_train = y_train.str.replace('.', '')  # Remove periods from labels
y_test = y_test.str.replace('.', '')    # Remove periods from labels
...
X_test_encoded = encoded[len(X_train): len(X_train) + X_test.shape[0]]
